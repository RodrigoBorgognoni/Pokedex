...
export function renderPokemonList(pokemons, container) {
    container.innerHTML += pokemons
        .map((pokemon) =>
                `<li class="pokemon ${pokemon.color}" data-name="${pokemon.name}" data-type="${pokemon.types.join(' ')}"> <span class="number">#${pokemon.id
                    .toString()
                    .padStart(3, '0')}</span> <span class="name">${capitalizeFirstLetter(
                    pokemon.name
